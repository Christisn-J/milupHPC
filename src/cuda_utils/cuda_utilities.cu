...
#if SINGLE_PRECISION
            return __isnanf(x);   // float
#else
            return __isnan(x);    // double
#endif
        }

        __device__ inline bool isInf(real x) {
#if SINGLE_PRECISION
            return __isinff(x);   // float
#else
            return __isinf(x);    // double
#endif
        }

        __device__ inline bool isOverflow(real x) {
#if SINGLE_PRECISION
            const real limit = Constants::numerics::dbl_max * 0.9f;
            return x > limit || x < -limit;
#else
            const real limit = Constants::numerics::dbl_max * 0.9;
            return x > limit || x < -limit;
#endif
        }
...
#if SINGLE_PRECISION
            const real tiny = Constants::numerics::dbl_min;
            return (x != 0.0f && fabsf(x) < tiny);
#else
            const real tiny = Constants::numerics::dbl_min;
            return (x != 0.0 && fabs(x) < tiny);
#endif
        }
